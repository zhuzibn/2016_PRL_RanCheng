...
if (0)%compare m1m2 & ml
    clear all;close all
    load('final-200LLG.mat')
    figure
    plot(tt*1e12,mmz(:,1)','-b','LineWidth',2);
    hold on
    clear all
    load('../AFMml/ml-200.mat');
...
if (0)%compare m1m2_LLG & mlm2_LL
    load('final-200LLG.mat')
    figure
    plot(tt*1e12,mmz(:,1)','-b','LineWidth',2);
    hold on
    clear all
    load('D:\tmp\2016PRLRanCheng\AFMm1m2_LL\final-200.mat');
    plot(tt*1e12,mmz(:,1)','-r','LineWidth',1);
