...
dmdt=gam/2*(cross(fL,L)+cross(fm,mmm))+alp/2*(cross(mmm,dmdt_tmp)+cross(L,dLdt_tmp))+...
    gam/2*(cross(L,cross(ws,L))+cross(mmm,cross(ws,mmm)));
dLdt=gam/2*(cross(fm,L)+cross(fL,mmm))+alp/2*(cross(mmm,dLdt_tmp)+cross(L,dmdt_tmp))+...
